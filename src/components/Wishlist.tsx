...
export const Wishlist: React.FC = () => {
  ...
  const removeLocal = (id: string) => {
    const next = wishlistServices.filter(x => x.id !== id);
    setWishlistServices(next);
    try {
      // persist local change immediately
      localStorage.setItem('local_bookmarks', JSON.stringify(next.map(x => x.id)));
      window.dispatchEvent(new CustomEvent('bookmarks:changed'));
      window.dispatchEvent(new CustomEvent('toast:show', { detail: { message: 'Removed from wishlist', type: 'success' } }));
    } catch (e) { }
