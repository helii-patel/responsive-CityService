...
const filteredServices = useMemo(() => {
  ...
  const toggleBookmark = async (serviceId: string) => {
    ...
    if (bookmarked.has(serviceId)) {
      ...
      const removedDetail = { message: 'Removed from wishlist', type: 'success' } as any;
      ...
      // notify other components/pages that bookmarks changed and show feedback even if localStorage fails
      window.dispatchEvent(new CustomEvent('bookmarks:changed'));
      const addedDetail = { message: 'Added to wishlist', type: 'success' } as any;
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (res.status === 404 || res.status === 500) {
              // Backend API not available - keep local bookmark but don't show additional message
              console.warn('Backend bookmark API not available, keeping local bookmark');
            } else {
              // Actual failure - revert optimistic UI
              newBookmarked.delete(serviceId);
              setBookmarked(newBookmarked);
              window.dispatchEvent(new CustomEvent('toast:show', { detail: { message: 'Failed to add to wishlist', type: 'error' } }));
            }
          }
        } catch (err) {
          ...
          if (err instanceof TypeError && err.message.includes('fetch')) {
            // Network error - backend probably not running, keep local bookmark but don't show additional message
